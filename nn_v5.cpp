...
class ANN
{
	...
	float **PrevBatchDeltaWeights;

	float **OldGradients; //Gradients of last batch used to adjust LRgain

	float **NewGradients; //Gradients of current batch used to adjust LRgain

	float **accumGradient; //accumulated Gradient of a batch while training in batch mode

	bool StartDLR;

	float **DynamicLr;	// To put A Learning Rate for each weight 

	float **LRgain; //learning rate gain for each connection

	float *Bias;

	float *DeltaBias;

	float *BatchDeltaBias;

	float *PrevBatchDeltaBias;
	...
	float *LayerLr;		// To Put A separate Learning Rate for each Layer.
	...
	void Compute_DeltasWithDLRandMoments(float*,float,int,int);

	void LRAdjust(float**,float**);

	float Compute_BatchDeltasAndError(float**,float**,float,int,int*);

	void Copy_Gradients(float**);

	void Copy_Deltas(float**,float*);

	void Set_DLR();

	void IncrementWeights();

	void BatchIncrementWeights();
	...
	void ANN::Intialise_Weights()
	{
		...
		*/
		...
		{
			//cout<<"Initializing layer<<LayerNo<<"...";
	...
	void ANN::ConfigureNetwork()
	{
		...
		*/
		...
		BatchDeltaWeights = new float*[TotalUnits];

		PrevBatchDeltaWeights = new float*[TotalUnits];

		LRgain = new float*[TotalUnits];

		OldGradients = new float*[TotalUnits];

		NewGradients = new float*[TotalUnits];

		accumGradient = new float*[TotalUnits];
		...
		if(0==Weights || 0==Output || 0==DeltaWeights || 0==LocalGradient ||
		   0 == BatchDeltaWeights || 0 == LRgain || 0 == PrevBatchDeltaWeights)
		{
		...
		for(int UnitNo=0;UnitNo<UnitsInLayer[0];UnitNo++)
		{
			...
			BatchDeltaWeights[UnitNo] = new float[InputDimension];

			PrevBatchDeltaWeights[UnitNo] = new float[InputDimension];

			LRgain[UnitNo] = new float[InputDimension];

			OldGradients[UnitNo] = new float[InputDimension];

			NewGradients[UnitNo] = new float[InputDimension];

			accumGradient[UnitNo] = new float[InputDimension];
			//DynamicLr[UnitNo] = new float[InputDimension];
			
			for(int k=0;k<InputDimension;k++)
			{
				DeltaWeights[UnitNo][k]=0.0;
				BatchDeltaWeights[UnitNo][k]=0.0;
				PrevBatchDeltaWeights[UnitNo][k]=0.0;
				LRgain[UnitNo][k] = 1.0;
				accumGradient[UnitNo][k]=0.0;
			}
		}
		...
		for(int LayerNo=1;LayerNo<TotalLayers;LayerNo++)
		{
			...
			{	
				...
				BatchDeltaWeights[SP+UnitNo]=new float[UnitsInLayer[LayerNo-1]];

				PrevBatchDeltaWeights[SP+UnitNo]=new float[UnitsInLayer[LayerNo-1]];

				LRgain[SP+UnitNo]=new float[UnitsInLayer[LayerNo-1]];

				OldGradients[SP+UnitNo] = new float[InputDimension];

				NewGradients[SP+UnitNo] = new float[InputDimension];

				accumGradient[SP+UnitNo] = new float[InputDimension];
				...
				for(int k=0; k < UnitsInLayer[LayerNo-1]; k++)
				{
					...
					BatchDeltaWeights[SP+UnitNo][k]=0.0;
					PrevBatchDeltaWeights[SP+UnitNo][k]=0.0;
					LRgain[SP+UnitNo][k]=1.0;
					accumGradient[SP+UnitNo][k]=0.0;
				}
			}
		}
		...
		BatchDeltaBias = new float[TotalUnits];
		PrevBatchDeltaBias = new float[TotalUnits];
		for(int UnitNo=0;UnitNo<TotalUnits;UnitNo++)
		{
			Bias[UnitNo]=0.0;
			DeltaBias[UnitNo]=0.0;
			BatchDeltaBias[UnitNo]=0.0;
			PrevBatchDeltaBias[UnitNo]=0.0;
		}
}
...
//		}
	...
	void ANN::BatchIncrementWeights()
		{
			int LayerNo,UnitNo,k;
			...
			for(UnitNo=0;UnitNo<UnitsInLayer[LayerNo];UnitNo++)
			{
				for(k=0;k<InputDimension;k++)
				{
					Weights[SP+UnitNo][k] = Weights[SP+UnitNo][k] + BatchDeltaWeights[SP+UnitNo][k];
					//cout<<"W = "<<Weights[SP+UnitNo][k]<<endl;

				}

				Bias[SP+UnitNo] = Bias[SP+UnitNo] +  BatchDeltaBias[SP+UnitNo];
			}
	...
	void ANN::FreeTheSpace()
	{
		...
		//delete []* DeltaWeights;
		cout << "Deleted output"<<endl;

		/*for(int i=0;i<TotalUnits;i++)
		{
			delete [] Weights[i];
			cout<<"delted weights of unit"<<i<<endl;
			delete [] DeltaWeights[i];
			cout<<"delted deltaweights of unit"<<i<<endl;
			delete [] BatchDeltaWeights[i];
			cout<<"delted batchdeltaweights of unit"<<i<<endl;
		}*/
		//cout << "Delted unitwise weights,deltaweights etc.."<<endl;
		delete [] Weights;
		cout << "Delted Weights"<<endl;
		delete [] DeltaWeights;
		cout << "Delted DeltaWeights"<<endl;
		delete [] BatchDeltaWeights;
		cout << "Delted batchdeltaweights"<<endl;
		delete [] LocalGradient;
		cout << "Delted localgradient"<<endl;
		delete [] ErrorAtOutputLayer;
		cout << "Delted erroratoutputlayer"<<endl;
		delete [] Bias;
		cout << "Delted bias"<<endl;
		delete [] DeltaBias;
		cout << "Delted deltabias"<<endl;
		delete [] BatchDeltaBias;
		cout << "Delted batchdeltabias"<<endl;
		delete [] accumGradient;
		cout << "Delted accumgradient"<<endl;
		delete [] OldGradients;
		cout << "Delted oldgradients"<<endl;
		delete [] NewGradients;
		cout << "Delted newgradients"<<endl;
		delete [] UnitsInLayer;
		cout << "Delted unitsinlayer"<<endl;
		delete [] TypeOfLayer;
		cout << "Delted typeoflayer"<<endl;
		delete [] StartingPosition;
		cout << "Delted startingposition"<<endl;
	}
	...
	void ANN::Compute_DeltasWithLayerLr(float *InputPattern,float AlphaPassed)
  	...
  	void ANN::Compute_DeltasWithDLRandMoments(float *InputPattern,float AlphaPassed,int BatchSize,int FrameNo)
    {
            int SP_thislayer;
            ...
            for(UnitNo=0;UnitNo<UnitsInLayer[LayerNo];UnitNo++)
            {
                    ...
                    {
						DeltaWeights[SP_thislayer + UnitNo][k]
						= LRgain[SP_thislayer+UnitNo][k]*Neta * InputPattern[k] * LocalGradient[SP_thislayer + UnitNo];
						BatchDeltaWeights[SP_thislayer + UnitNo][k]
						+= DeltaWeights[SP_thislayer + UnitNo][k]/BatchSize;
						accumGradient[SP_thislayer + UnitNo][k]
						+= (InputPattern[k]*LocalGradient[SP_thislayer + UnitNo])/BatchSize;
					}
                    	DeltaBias[SP_thislayer + UnitNo]
                    	= LRgain[SP_thislayer+UnitNo][k]*Neta * LocalGradient[SP_thislayer + UnitNo];
                    	BatchDeltaBias[SP_thislayer + UnitNo]
                    	+= DeltaBias[SP_thislayer + UnitNo]/BatchSize;
            }
            ...
            for(LayerNo=1;LayerNo<TotalLayers;LayerNo++)
            {
                    ...
                    for(UnitNo=0;UnitNo<UnitsInLayer[LayerNo];UnitNo++)
                    {
						...
						{
							DeltaWeights[SP_thislayer + UnitNo][k]
							= LRgain[SP_thislayer+UnitNo][k]*Neta * Output[SP_prevlayer + k]*LocalGradient[SP_thislayer + UnitNo];
							BatchDeltaWeights[SP_thislayer + UnitNo][k]
							+= DeltaWeights[SP_thislayer + UnitNo][k]/BatchSize;
							accumGradient[SP_thislayer + UnitNo][k]
							+= (InputPattern[k]*LocalGradient[SP_thislayer + UnitNo])/BatchSize;
            ...
            if(LastFrame){
            	for(LayerNo=1;LayerNo<TotalLayers;LayerNo++)
				{
						...
						for(UnitNo=0;UnitNo<UnitsInLayer[LayerNo];UnitNo++)
						{
							...
							{
								BatchDeltaWeights[SP_thislayer + UnitNo][k]
								+= AlphaPassed * PrevBatchDeltaWeights[SP_thislayer + UnitNo][k];
							}
							BatchDeltaBias[SP_thislayer + UnitNo]
						    += AlphaPassed * PrevBatchDeltaBias[SP_thislayer + UnitNo];
						}
				}
            }
    }
	...
	float ANN::Compute_BatchDeltasAndError(float **BGInputLPCC,float **BGOutputLPCC,float AlphaPassed,
											int BatchSize,int *FrameNumbers)
	        {
	                int SP_thislayer;
	                ...
	                Copy_Deltas(PrevBatchDeltaWeights,PrevBatchDeltaBias); // copy the Delta weights and biases of the last batch
	                // Initialize deltas with zeros
	                //cout<<"Initializing batch deltas with zeros...";
	                for(int UnitNo=0;UnitNo<UnitsInLayer[0];UnitNo++)
	                {
	                	for(int k=0;k<InputDimension;k++)
	                	{
	                ...
	                if (StartDLR)
	                {
	                	Copy_Gradients(OldGradients);
	                }
	                for(int FrameNo=0;FrameNo<BatchSize;FrameNo++)
	                {
	                	//cout<<"Here 1"<<endl;
	                	InputPattern = BGInputLPCC[FrameNumbers[FrameNo]];
	                	//cout<<"Input pattern read:"<<endl<<InputPattern<<endl;
	                	OutputPattern = BGOutputLPCC[FrameNumbers[FrameNo]];
	                	//cout<<"output pattern read:"<<endl<<OutputPattern<<endl;
	                	Compute_Output(InputPattern);
	                	//cout<<"output computed"<<endl;
	                	Compute_Error(OutputPattern,&error);
	                	//cout<<"error:"<<error<<endl;
	                	//cout<<"error computed"<<endl;
	                	BatchError = BatchError + error;
	                	//cout<<"BatchError:"<<BatchError<<endl;
	                	Compute_LocalGradients();
	                	//cout<<"local gradients computed"<<endl;
	                	Compute_DeltasWithDLRandMoments(InputPattern,AlphaPassed,BatchSize,FrameNo);
	                	//cout<<"deltas computed"<<endl;
/*	                	LayerNo=0;
   		                SP_thislayer = StartingPosition[LayerNo];
   		                for(UnitNo=0;UnitNo<UnitsInLayer[0];UnitNo++)
   		                {
   		                	for(k=0;k<InputDimension;k++)
   		                	{
   		                		BatchDeltaWeights[SP_thislayer + UnitNo][k] += (DeltaWeights[SP_thislayer + UnitNo][k])/BatchSize;
   		                		accumGradient[SP_thislayer + UnitNo][k] += InputPattern[k]*LocalGradient[SP_thislayer + UnitNo]/BatchSize;
   		                	}
   		                	BatchDeltaBias[SP_thislayer + UnitNo] += (DeltaBias[SP_thislayer + UnitNo])/BatchSize;
   		                }
	                	...
	                	for(LayerNo=1;LayerNo<TotalLayers;LayerNo++)
	                	{
	                		...
	                		for(UnitNo=0;UnitNo<UnitsInLayer[LayerNo];UnitNo++)
	                		{
	                			...
	                			{
	                				BatchDeltaWeights[SP_thislayer + UnitNo][k] +=  (DeltaWeights[SP_thislayer + UnitNo][k])/BatchSize;
	                				accumGradient[SP_thislayer + UnitNo][k] += Output[SP_prevlayer + k]* LocalGradient[SP_thislayer + UnitNo]/BatchSize;
	                			}
	                			BatchDeltaBias[SP_thislayer + UnitNo] +=  (DeltaBias[SP_thislayer + UnitNo])/BatchSize;
	                		}
	                	}*/
	                ...
	                if (StartDLR)
	                {
	                	Copy_Gradients(NewGradients);
	                	LRAdjust(OldGradients,NewGradients);
	                }
	                StartDLR = 1;
	                return BatchError;
	...
	void ANN::Copy_Gradients(float **Gradients)
	{//Copies the current gradients in LocalGradeint to Gradients passed as input
		int SP_thislayer,SP_prevlayer,LayerNo;
		LayerNo=0;
		...
		for(int UnitNo=0;UnitNo<UnitsInLayer[LayerNo];UnitNo++)
		{
				for(int k=0;k<InputDimension;k++)
				{
						Gradients[SP_thislayer + UnitNo][k]
...
void ANN::Copy_Deltas(float **DeltaWts,float *DeltaBs)
{//Copies the current gradients in LocalGradeint to Gradients passed as input
	int SP_thislayer,SP_prevlayer,LayerNo;
	LayerNo=0;
	...
	for(int UnitNo=0;UnitNo<UnitsInLayer[LayerNo];UnitNo++)
	{
		for(int k=0;k<InputDimension;k++)
		{
			DeltaWts[SP_thislayer + UnitNo][k]
			= BatchDeltaWeights[SP_thislayer + UnitNo][k];
		}
		DeltaBs[SP_thislayer + UnitNo] = BatchDeltaBias[SP_thislayer + UnitNo];
	}
	for(LayerNo=1;LayerNo<TotalLayers;LayerNo++)
	{
		SP_thislayer = StartingPosition[LayerNo];
	...
	void ANN::LRAdjust(float **OldGradients,float **NewGradients)
	{ //Adjusts the learning rate gain(LRgain) using the sign of the gradients
		for(int UnitNo=0;UnitNo<UnitsInLayer[0];UnitNo++)
		{
			for(int k=0;k<InputDimension;k++)
			{
				if(OldGradients[UnitNo][k]*NewGradients[UnitNo][k]>0)
				{
					//cout<<"increasing gain"<<endl;
					LRgain[UnitNo][k]+=0.05;
				}
				else
				{
					//cout<<"decreasing gain"<<endl;
					LRgain[UnitNo][k]*=0.95;
				}
			}
		}
		for(int LayerNo=1;LayerNo<TotalLayers;LayerNo++)
		{
			...
			for(int UnitNo=0;UnitNo<UnitsInLayer[LayerNo];UnitNo++)
			{
				for(int k=0; k < UnitsInLayer[LayerNo-1]; k++)
				{
					if(OldGradients[SP+UnitNo][k]*NewGradients[SP+UnitNo][k]>0)
					{
						//cout<<"increasing gain"<<endl;
						LRgain[SP+UnitNo][k]+=0.05;
					}
					else
					{
						//cout<<"decreasing gain"<<endl;
						LRgain[SP+UnitNo][k]*=0.95;
					}
				}
			}
		}
